...
class NativeStrategy(Strategy):
    ...
    def _gen_pystr(self, ctx: GenerationContext) -> np.ndarray:
        """Random alphanumeric strings — no Faker required."""
        chars = np.frombuffer(b"abcdefghijklmnopqrstuvwxyz0123456789", dtype="S1")
        length = 12
        indices = ctx.rng.integers(0, len(chars), size=(ctx.row_count, length))
        return np.array(
            ["".join(c.decode() for c in chars[row]) for row in indices],
            dtype=object,
        )
