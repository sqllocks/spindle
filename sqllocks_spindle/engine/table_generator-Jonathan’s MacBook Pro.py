...
class TableGenerator:
    ...
    def generate(
        ...
        # Build DataFrame — only schema-defined columns (ctx.current_table may
        # contain internal cache entries like _rs_* from record_sample strategy)
        df = pd.DataFrame({col: ctx.current_table[col] for col in table.columns})
    ...
    def _order_columns(self, table: TableDef) -> list[str]:
        ...
        for col_name, col in table.columns.items():
            ...
            if col_name in table.primary_key and strategy in ("sequence", "uuid"):
                ...
                "conditional", "correlated", "self_referencing", "self_ref_field",
