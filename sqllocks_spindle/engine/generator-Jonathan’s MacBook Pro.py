...
class Spindle:
    ...
    def generate(
        ...
        Args:
            ...
            verbose: Show progress bar during generation (default True).
        """
        # Resolve schema
        ...
        tables_to_generate = [t for t in gen_order if t in parsed.tables]
        total = len(tables_to_generate)

        for step, table_name in enumerate(tables_to_generate, start=1):
            count = row_counts.get(table_name, 100)

            if verbose:
                self._print_progress(step, total, table_name, count)
        ...
        if verbose:
            # Move to new line after last progress update
            print()

        # Compute phase: back-fill computed columns (order_total, refund_amount, etc.)
        self._compute_phase(tables, parsed)

        # Business rules phase: fix constraint violations
    ...
    def _print_progress(self, step: int, total: int, table_name: str, count: int) -> None:
        """Print an in-place progress bar to stdout."""
        bar_width = 30
        filled = int(bar_width * step / total)
        bar = "█" * filled + "░" * (bar_width - filled)
        pct = int(100 * step / total)
        label = f"{table_name:<25}"
        print(f"\r  [{bar}] {pct:>3}%  {label}  {count:>8,} rows  ({step}/{total})", end="", flush=True)
    ...
    def _compute_phase(
        ...
        """Back-fill computed columns from child or parent table data."""
        for table_name, table_def in schema.tables.items():
            ...
            for col_name, col in table_def.columns.items():
                ...
                if rule == "lookup_parent":
                    # Parent-lookup: this table has the FK; copy a value from the parent.
                    # Config: parent_table, parent_column, via (FK column in this table)
                    parent_table = config.get("parent_table", "")
                    parent_column = config.get("parent_column", "")
                    via = config.get("via", "")
                    if not all([parent_table, parent_column, via]):
                        continue
                    if parent_table not in tables:
                        continue
                    # Find PK of the parent table
                    parent_pk = schema.tables[parent_table].primary_key
                    if not parent_pk:
                        continue
                    ComputedStrategy.backfill(
                        parent_df=tables[table_name],   # child table (has the FK)
                        child_df=tables[parent_table],  # parent table (has the value)
                ...
                # Child-aggregation rules (sum_children, count_children, etc.)
